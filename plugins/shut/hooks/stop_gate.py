...
def check(blocks: list[tuple[str, bool]]) -> str | None:
    ...
    for idx, (text, _) in enumerate(blocks):
        ...
        if not final:
            ...
            if not body.rstrip().endswith("?") and (len(words) > LABEL_WORDS or BAD_PUNCT.search(body)):
                labels.append(" ".join(words[:6]))
            continue

        if any(HEADING.match(ln) for ln in lines):
            return with_labels("The answer used a markdown heading. shut bans headings. Rewrite it as plain sentences.", labels)
        if sum(1 for ln in lines if TABLE.match(ln) and ln.count("|") >= 2):
            return with_labels("The answer used a table. shut bans tables. Rewrite it as plain sentences.", labels)
